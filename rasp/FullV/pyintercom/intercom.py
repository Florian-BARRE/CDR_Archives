...
from .utils import DataEvent, ThreadSafeDict
...
CURRENT_SEND_PROTOCOL_VERSION = b"\x01"
...
class Intercom:
    ...
    def __intercom_thread(self):
        ...
        while True:
            try:
                ...
                if len(received_bytes) > (3 + 4 * 2 + 1) and received_bytes[:4] == PACKET_START and received_bytes[-4:] == PACKET_END:
                    protocol_ver = received_bytes[4]
                    if protocol_ver != 1:
                        raise RuntimeError("Unsupported protocol version: " + str(protocol_ver))

                    topic_code = int.from_bytes(received_bytes[5:8], "big")
                    received_data = json.loads(received_bytes[8:-4])
                ...
                print("Intercom thread error:", exception)
    ...
    def start(self):
        ...
        if self.receive_thread is None:
            ...
            if not self.socket_ready.wait(10):
                raise RuntimeError("Intercom socket setup timed out!")
    ...
    def wait_here(self):
        ...
        while True:
            with self.__message_received:
                if self.__message_received.wait(1):  # restart the wait operation every second
                    self.run_callbacks()
    ...
    def publish(self, topic: str, message_data: Any = None) -> None:
        ...
        self.com_socket.sendto(PACKET_START + CURRENT_SEND_PROTOCOL_VERSION + topic_info[0].to_bytes(3, "big") + bytes(json.dumps(message_data, separators=(",", ":")), "utf-8") + PACKET_END, (topic_info[1], MULTICAST_PORT))

    def publish_raw(self, topic_code: int, message_data: Any = None) -> None:
        self.start()

        topic_ip = self.__topic_code_to_ip(topic_code)
        self.com_socket.sendto(PACKET_START + CURRENT_SEND_PROTOCOL_VERSION + topic_code.to_bytes(3, "big") + bytes(json.dumps(message_data, separators=(",", ":")), "utf-8") + PACKET_END, (topic_ip, MULTICAST_PORT))
