...
class Camera_Calibrator:
    ...
    def __init__(self, cal_picture_folder_path: str, chess_size: tuple[int, int], picture_ext: str = "jpg"):
        ...
        picture_path = f"{path.join(self.cal_picture_folder_path, '*')}.{picture_ext}"
        self._pictures_path_list = glob.glob(picture_path)

        if self._pictures_path_list == []:
            print(f"WARNING: no pictures found ! Path: {picture_path}")
    ...
    def save_undistort_coef(self, save_path):
        with open(f"{save_path}.json", "w") as file:
            content = {
                "cam_matrix": self._cam_matrix.tolist(),
                "distortion_coef": self._distortion_coef.tolist(),
                "optimized_cam_matrix": self._optimized_cam_matrix.tolist()
            }
            content = str(content).replace("'", '"')
            file.write(content)

    def load_undistort_coef(self, store_path):
        if os.path.exists(store_path):
            with open(store_path) as file:
                data = json.load(file)
                self._cam_matrix = np.array(data.get("cam_matrix"))
                self._distortion_coef = np.array(data.get("distortion_coef"))
                self._optimized_cam_matrix = np.array(data.get("optimized_cam_matrix"))
        else:
            print(f"ERROR : load path not found ! Path : {store_path}")