...
Rolling_Basis::Rolling_Basis(unsigned short encoder_resolution, float center_distance, float wheel_diameter, byte max_pwm, unsigned short corrector_error_auth)
{
...
void Rolling_Basis::odometrie_handle(){
    ...
    if (abs(delta_left) > 3 || abs(delta_right) > 3)
        this->last_running_time = millis();
        
    this->IS_RUNNING = (millis() - this->last_running_time < 2000);
}
...
void Rolling_Basis::action_handle(Action *current_action) {
    ...
    // Anti static check
    if (current_action->start_rotation || current_action->move_forward || current_action->end_rotation){
        // There is static position ?
        if (!this->IS_RUNNING){
            current_action->antistatic_cpt++;
        }
        else{
            current_action->antistatic_cpt = 0;
            this->left_motor->_threshold_pwm_value = 0;
            this->right_motor->_threshold_pwm_value = 0;
        }
        // Increase pwm value
        if(current_action->antistatic_cpt > current_action->antistatic_trigger){
            this->left_motor->_threshold_pwm_value++;
            this->right_motor->_threshold_pwm_value++;

            if (this->left_motor->_threshold_pwm_value + 1 > 253)
                this->left_motor->_threshold_pwm_value = 254;

            if (this->right_motor->_threshold_pwm_value + 1 > 253) 
                this->right_motor->_threshold_pwm_value = 254;
        }
    }
