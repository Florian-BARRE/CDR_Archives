...
Action action0 = create_action(150.0, 0.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
Action action1 = create_action(150.0, -47.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
Action action2 = create_action(230.0, -47.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
Action action3 = create_action(5.0, 5.0 , NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
/*
  ...
  if (millis() < STOP_MOTORS_DELAY)
  {
    ...
    if (digitalRead(pin_on_off))
    {
      ...
      if (millis() > RETURN_START_POSITION_DELAY){
        ...
        if (dist_robot_start_position > 10.0)
