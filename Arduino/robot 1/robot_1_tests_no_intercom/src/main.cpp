...
Action action1 = create_action(30.0, 0.0);
Action action2 = create_action(30.0, 30.0);
...
void loop()
{
    ...
    // Check if the robot is authorize to move
    if (digitalRead(pin_on_off) && action_index < BALADE_SIZE)
    {
    ...
    Serial.println(
        ...
        String(" TICKS CURSOR :") + String(balade[action_index].ticks_cursor) +
