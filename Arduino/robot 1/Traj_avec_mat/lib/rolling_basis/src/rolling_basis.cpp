...
void Rolling_Basis::is_running_update(){
    if ((millis() - this->last_running_check) > 10){
        ...
        // Check if there is a little right or left deplacement
...
void Rolling_Basis::motors_controller(Action *action)
{
    ...
    // If max pwm is not set for the action we take the default rolling basis max pwm
    this->right_motor->handle(
        deltaT,
        (action->right_ref + action->right_sign * action->ticks_cursor),
        (action->max_pwm == 0) ? this->max_pwm : action->max_pwm
    );
    this->left_motor->handle(
        deltaT,
        (action->left_ref + action->left_sign * action->ticks_cursor),
        (action->max_pwm == 0) ? this->max_pwm : action->max_pwm
    );
}
...
{
    float delta_x = target_x - this->X;
    float delta_y = target_y - this->Y;
    float cmd_theta;
    // Extremum rotation
    // delta y null
    if (delta_y == 0)
