...
#define TRAJECTORY_PRECISION 40
#define NEXT_POSITION_DELAY 10000
#define INACTIVE_DELAY 400000
...
Action action1 = create_action(50.0, 0.0 , NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
...
int return_start_position_delay = 400000;
...
void loop()
{
  ...
  if (digitalRead(pin_on_off))
  {
    ...
    if (millis() > (long)return_start_position_delay)
