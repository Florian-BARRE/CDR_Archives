...
void Rolling_Basis::odometrie_handle(){
    ...
    this->IS_RUNNING = (millis() - this->last_position_update < 2000);
}
...
void Rolling_Basis::action_handle(Action *current_action) {
    ...
    Serial.println(String(" BOUGE ! State ruuning : ") + String(this->IS_RUNNING));
    if (
        (current_action->start_rotation || current_action->move_forward || current_action->end_movement_cpt) &&
        !this->IS_RUNNING
    ){
        this->left_motor->_threshold_pwm_value  = this->max_pwm;
        this->right_motor->_threshold_pwm_value = this->max_pwm;
    }
    else{
