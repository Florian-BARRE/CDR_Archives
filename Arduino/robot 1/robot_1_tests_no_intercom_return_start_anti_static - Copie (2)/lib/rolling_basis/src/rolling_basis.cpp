...
Rolling_Basis::Rolling_Basis(unsigned short encoder_resolution, float center_distance, float wheel_diameter, byte max_pwm, unsigned short corrector_error_auth, unsigned short inactive_delay)
{
...
float Rolling_Basis::trajectory_theta_calculator(float target_x, float target_y)
{
    ...
    */
    ...
    // Any cases
        ...
        /*
        Serial.println("Any cases");
        Serial.println(
        String(" delta_x : ") + String(delta_x) +
        String(" delta_y : ") + String(delta_y) );
        cmd_theta = atan2(delta_y, delta_x);
        
        // Determine theta sign 
        if(target_y > this->Y)
            cmd_theta = 1.0 * cmd_theta;
        else
            cmd_theta = -1.0 * cmd_theta;
        */
        // Add current theta
    ...
    Serial.println(
        String(" delta_x : ") + String(delta_x) +
        String(" delta_y : ") + String(delta_y) +
        String(" cmd_theta : ") + String(cmd_theta) +
        String(" this->THETA : ") + String(this->THETA) +
        String(" DELTA THETA : ") + String(cmd_theta));
    ...
    if (action->mesurement_taker) {
        ...
        if(action->start_rotation){
            ...
            Serial.println(
                ...
                String(" Y ") + String(this->Y) +
                String(" THETA ") + String(this->THETA) +
                String(" TX ") + String(action->target_x) +
                String(" TY ") + String(action->target_y) +
                String(" cmd_theta ") + String(cmd_theta));
            ...
            Serial.println(
                ...
                String(" TX ") + String(action->target_x) +
                String(" TY ") + String(action->target_y) +
                String(" cmd_dist ") + String(cmd_dist));
